...
-- Compatibile con TopHost (MariaDB/MySQL condivisi). Eseguire una sola volta.
-- ============================================================

ALTER TABLE `saved_searches`
  ADD COLUMN `vtype`         VARCHAR(80) DEFAULT NULL AFTER `q`,
  ADD COLUMN `confirm_token` CHAR(32)    DEFAULT NULL AFTER `token`,
  ADD COLUMN `confirmed_at`  DATETIME    DEFAULT NULL AFTER `confirm_token`;

-- Gli alert dei guest partono solo dopo la conferma: l'indice serve al cron,
-- che ora filtra anche su confirmed_at.
ALTER TABLE `saved_searches`
  ADD INDEX `idx_confirm` (`confirm_token`),
