# Allonwheel — Admin: cancellazione record nelle tabelle non ancora controllate
*Rev. 2 giu 2026 — aggiunta azione Delete (annunci free/premium e aziende) con cascata sui figli, file cleanup, CSRF e audit.*

## Analisi dell'utente admin (cosa controllava prima)

| Tabella | Prima | Ora |
|---|---|---|
| `vehicle_types` | CRUD completo | invariato |
| `users` | lettura + grant/revoke premium | invariato (vedi nota) |
| `06_company` | solo activate/deactivate | **+ Delete** (con figlie) |
| `06_company_gallery` / `_products` / `_products_special` / `_services` | non gestite | **cancellate in cascata** con l'azienda |
| `02_free_ads` / `03_ads` | solo approve/reject | **+ Delete** (con figlie) |
| `02_free_ads_gallery` / `03_ads_gallery` / `03_ads_tech_details` | non gestite | **cancellate in cascata** con l'annuncio |
| `admin_audit_log` / `consent_log` / `login_attempts` | log | **lasciate immutabili** (vedi nota) |

## Cosa è stato aggiunto

- **`_admin/moderate_ads.php`**: nuova azione `delete` (oltre ad approve/reject). Cancella l'annuncio + gallery, e per i premium anche `03_ads_tech_details`, poi rimuove i file immagine (copertina + gallery, original/thumbnail) con protezione path-traversal. Tutto in transazione, con voce `admin_audit_log` (`action='delete_ad'`) e pulsante **Delete** con conferma.
- **`_admin/manage_companies.php`**: nuova azione `delete` (oltre ad activate/deactivate). Cancella in cascata `06_company_gallery`, `06_company_products`, `06_company_products_special`, `06_company_services` e infine `06_company`, rimuovendo logo e immagini gallery (con fallback ai vecchi upload flat). Transazione, audit (`action='delete_company'`), pulsante **Delete** con conferma.

Il pattern di cancellazione (transazione + cleanup file con `realpath`/anti path-traversal) riprende quello già collaudato in `02_free_ads/02_delete_ad.php`, ma a livello admin (nessun vincolo di proprietà).

## Note (scelte di sicurezza)

- **Log non cancellabili**: `admin_audit_log` (tracciabilità), `consent_log` (GDPR) e `login_attempts` (sicurezza) restano volutamente immutabili. Cancellarli indebolirebbe audit/compliance.
- **`users`**: l'eliminazione di un utente cascata su annunci e azienda; esiste già `01_login/delete_account.php` per l'auto-cancellazione. Se vuoi un Delete utente lato admin (con relativa cascata), lo aggiungo su richiesta.
- **Insert**: per annunci/aziende l'inserimento è lato utente (non ha senso lato admin); l'unica tabella-catalogo con insert admin è `vehicle_types`, già gestita.
