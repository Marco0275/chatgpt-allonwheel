...
//
// 23 lug 2026. La modifica e' divisa per sezione (02_modify_road.php,
// 02_modify_special.php, 02_modify_shelter.php) perche' ogni sezione ha
...
try {
    $st = $pdo->prepare('SELECT item_kind, macro_category FROM `02_free_ads` WHERE id_ads = :id AND id_user = :u LIMIT 1');
    $st->execute([':id' => $id_ads, ':u' => $user_id]);
    $ad = $st->fetch(PDO::FETCH_ASSOC);
} catch (Throwable $e) {
    error_log('[Allonwheel] 02_modify dispatcher: ' . $e->getMessage());
    $ad = null;
}
...
header('Location: 02_modify_' . $section . '.php?id_ads=' . $id_ads);
exit;
