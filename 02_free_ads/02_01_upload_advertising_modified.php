<?php
// ============================================================
// 02_free_ads/02_01_upload_advertising_modified.php (Phase 4)
// Handler POST per il salvataggio della modifica annuncio free.
// ============================================================
...
// Classificazione: RISPECCHIA 02_01_upload_advertising.php (22 lug 2026).
// item_kind e macro_category arrivano dai campi hidden del form (valori
// dell'annuncio, non modificabili); il tipo veicolo deve appartenere alla
// macro (isValidType), come in inserimento. La famiglia (product_macro) NON
// si prende piu' dal POST: e' DERIVATA piu' sotto con ProductMacro::forAd(),
// esattamente come fa l'inserimento.
...
if ($item_kind === VehicleTaxonomy::KIND_SHELTER) {
  // Shelter/Container e' sempre Special, con tipo dedicato (come inserimento)
...
if ($vehicle_type === '' || ($item_kind !== VehicleTaxonomy::KIND_SHELTER
  ...
  header('Location: 02_modify_insert_ad.php?id_ads=' . $id_ads);
  exit;
}
...
// Famiglia DERIVATA come nell'inserimento (non scelta a mano nel form).
...
// Update — la WHERE include sempre id_user (defense in depth)
$sql = 'UPDATE `02_free_ads` SET
  title = :title,
  subtitle = :subtitle,
...
try {
  ...
  error_log('[Allonwheel] Modify free ad error: ' . $e->getMessage());
  $_SESSION['error_message'] = 'Database error while saving changes.';
  header('Location: 02_modify_insert_ad.php?id_ads=' . $id_ads);
  exit;
}

// Wizard completato: rotate il token persistente per la prossima azione
...
if (!empty($_FILES['ad_image']) && ($_FILES['ad_image']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE) {
  ...
  $aow_old = null;
  try {
    $aow_st = $pdo->prepare('SELECT image_original, image_thumbnail FROM `02_free_ads` WHERE id_ads = :a AND id_user = :u LIMIT 1');
  ...
  $aow_res = UploadHelper::handleImageUpload($_FILES['ad_image'], [
    'target_dir_original'  => '/upload_image/02_free_ads/original/',
    'target_dir_thumbnail' => '/upload_image/02_free_ads/thumbnail/',
    'thumb_width'    => 220,
    'thumb_height'   => 150,
    'thumb_crop'     => false,
  ...
  if ($aow_old) {
    $aow_root = rtrim($_SERVER['DOCUMENT_ROOT'] ?? '', '/') . '/upload_image/02_free_ads/';
  ...
  try {
    $pdo->prepare('UPDATE `02_free_ads` SET image_original = :o, image_thumbnail = :t WHERE id_ads = :a AND id_user = :u')
