...
// Pagina dedicata: mostra SOLO i veicoli "road" (categoria forzata,
// indipendente dal parametro GET).
// Coerenza URL: ?macro=special su questa pagina -> pagina Special.
if (($_GET['macro'] ?? '') === 'special') {
    $qs = $_GET; unset($qs['macro']);
    header('Location: special_vehicles.php' . ($qs ? '?' . http_build_query($qs) : ''), true, 301);
    exit;
}

$aow_rfq_section = 'road'; // RFQ dedicata di sezione (CTA in sidebar)
$macro = 'road';

// ---- Filtro vehicle_type (live da DB: solo tipi di questo macro) ----
$all_vtypes = [];
// Nuova tassonomia (24 lug 2026): questa pagina pesca da
// vehicle_types: la lista del codice della strada.
// La tabella non si sceglie qui: la decide VehicleTaxonomy.
require_once __DIR__ . '/libs/vehicle_taxonomy.class.php';
$all_vtypes = VehicleTaxonomy::typesForCategory(VehicleTaxonomy::CAT_ROAD, $pdo);
...
$qs = function (array $p) {
    ...
    return $pairs ? '?' . implode('&amp;', $pairs) : 'road_vehicles.php';
};
...
<?php
$seo_canonical = 'road_vehicles.php' . ($active_vtype !== '' ? '?vtype=' . rawurlencode($active_vtype) : '');
